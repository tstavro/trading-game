...
class discreteQAgent():
    ...
    def __assign_bins(self, observation):
        '''
    ...
    def __get_state_as_string(self, state):
        a2 = 0
    ...
    def __get_all_states_as_string(self):
        states = []
    ...
    def __initialize_Q_table(self):
        Q = {}
        all_states = self.__get_all_states_as_string()
    ...
    def play_game(self, Q_table, eps=0.02):
        observation = self.env.reset()
        done = False
        cnt = 0  # number of moves in an episode
        state_numeric = self.__assign_bins(observation)
        state = self.__get_state_as_string(state_numeric)
        total_reward = 0
        while not done:
            cnt += 1
            if np.random.uniform() < eps:
                ...
                act = self.__max_dict(Q_table[state])[0]  
            observation, reward, done, _ = self.env.step(act)
            total_reward += reward

            state_numeric = self.__assign_bins(observation)
            state_new = self.__get_state_as_string(state_numeric)
            state = state_new
        return total_reward, cnt


    def __Q_learn(self, Q, eps=0.5):
        '''
        ...
        while not done:
            ...
            if np.random.uniform() < eps:
                ...
                act = self.__max_dict(Q[state])[0] 
            observation, reward, done, _ = self.env.step(act)
            total_reward += reward
            state_numeric = self.__assign_bins(observation)
            state_new = self.__get_state_as_string(state_numeric)
            max_q_s1a1 = self.__max_dict(Q[state_new])[1]  
    ...
    def train(self, episodes=4000, eps_strategy=3):
        ...
        for n in range(episodes):
            if eps_strategy == 1:
                ...
                eps = 1.0 / np.sqrt(n + 1)
            episode_reward, episode_length = self.__Q_learn(Q, eps)
            if n % 10 == 0:
